...
const ParentVisitSlots = () => {
  ...
  const fetchSlots = async (daycareId) => {
    try {
      const response = await axiosInstance.get(`/visitSlots/daycare/${daycareId}`)
  ...
  const handleBook = async () => {
    try {
      console.log('Slot:', selectedSlot?._id)
      console.log('Child:', selectedChild)

      await axiosInstance.put(`/visitSlots/${selectedSlot._id}/book`, {
  ...
  const handleCancelBooking = async () => {
    ...
    try {
      await axiosInstance.put(`/visitSlots/${selectedBooking._id}/cancel`)

      toast.success('Visit booking cancelled successfully')
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-dc-mist via-dc-mist-2 to-dc-mist p-8 font-nunito">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          <h1 className="text-2xl font-semibold font-baloo text-dc-ink">My Visits</h1>
          <p className="text-dc-muted mt-1 text-sm">Book and manage your daycare visits.</p>
        </div>
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-[2rem] p-6 mb-8 shadow-[0_20px_50px_-12px_rgba(74,144,164,0.15)] border border-white/60">
          ...
          <h2 className="text-xl font-semibold font-baloo text-dc-ink mb-1">Available Visit Slots</h2>
          <p className="text-dc-muted mb-6 text-sm">Select a daycare to view available visit slots.</p>

          <div className="mb-6">
            <label className="block text-sm font-semibold text-dc-ink mb-2">Select Daycare</label>
            <select
            ...
            >
              ...
              {daycares.map((daycare) => (
                <option key={daycare._id} value={daycare._id}>{daycare.name}</option>
              ))}
            </select>
          </div>

          {!selectedDaycare ? (
            <p className="text-dc-muted text-sm">Please select a daycare first.</p>
          ) : slots.length === 0 ? (
            <p className="text-dc-muted text-sm">No slots available.</p>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-dc-border text-left text-dc-muted">
                    ...
                    <th className="py-3 pr-4 font-semibold">End Time</th>
                ...
                <tbody>
                  {slots.map((slot) => (
                    <tr key={slot._id} className="border-b border-dc-border/60 last:border-0">
                      <td className="py-3 pr-4 text-dc-ink">{new Date(slot.date).toLocaleDateString()}</td>
                      <td className="py-3 pr-4 text-dc-ink">{slot.startTime}</td>
                      <td className="py-3 pr-4 text-dc-ink">{slot.endTime}</td>
                      <td className="py-3">
                        <button
                          onClick={() => {
                            setSelectedSlot(slot)
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-dc-ink/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-[2rem] shadow-[0_20px_50px_-12px_rgba(74,144,164,0.35)] w-full max-w-md">
              ...
              <div className="flex items-center justify-between p-6 border-b border-dc-border">
                <h2 className="text-xl font-semibold font-baloo text-dc-ink">Book Visit</h2>
                <button
                  onClick={() => {
                    setShowBookingModal(false)
                    setSelectedSlot(null)
              ...
              <div className="p-6 space-y-5">
                ...
                <div>
                  <h3 className="font-semibold font-baloo text-dc-blue mb-2">Visit Details</h3>
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-[2rem] p-6 shadow-[0_20px_50px_-12px_rgba(74,144,164,0.15)] border border-white/60">
          ...
          <div className="mb-6">
            <h2 className="text-xl font-semibold font-baloo text-dc-ink">My Booked Visits</h2>
            <p className="text-dc-muted mt-1 text-sm">View and manage your booked daycare visits.</p>
          </div>

          {myBookings.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-dc-muted text-sm">You have no booked visits.</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-dc-border text-left text-dc-muted">
                    <th className="py-3 pr-4 font-semibold">Daycare</th>
                    <th className="py-3 pr-4 font-semibold">Date</th>
                    <th className="py-3 pr-4 font-semibold">Start Time</th>
                    <th className="py-3 pr-4 font-semibold">End Time</th>
                ...
                <tbody>
                  {myBookings.map((booking) => (
                    <tr key={booking._id} className="border-b border-dc-border/60 last:border-0">
                      <td className="py-3 pr-4 text-dc-ink">{booking.daycare?.name || 'Daycare'}</td>
                      <td className="py-3 pr-4 text-dc-ink">{new Date(booking.date).toLocaleDateString()}</td>
                      <td className="py-3 pr-4 text-dc-ink">{booking.startTime}</td>
                      <td className="py-3 pr-4 text-dc-ink">{booking.endTime}</td>
        ...
        {showDetailsModal && selectedBooking && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-dc-ink/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-[2rem] shadow-[0_20px_50px_-12px_rgba(74,144,164,0.35)] w-full max-w-lg max-h-[90vh] overflow-y-auto">
              ...
              <div className="flex items-center justify-between p-6 border-b border-dc-border">
                <h2 className="text-xl font-semibold font-baloo text-dc-ink">Visit Details</h2>
                <button
                  onClick={() => {
                    setShowDetailsModal(false)
                    setSelectedBooking(null)
              ...
              <div className="p-6 space-y-6">
                ...
                <div>
                  <h3 className="font-semibold font-baloo text-dc-blue mb-3">Daycare Details</h3>
                  <div className="bg-dc-hover rounded-2xl p-4 space-y-2 text-sm text-dc-ink">
                    <p><strong>Name:</strong> {selectedBooking.daycare?.name || 'Not available'}</p>
                    <p><strong>Address:</strong> {selectedBooking.daycare?.address || 'Not available'}</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="bg-dc-hover rounded-2xl p-4 space-y-2 text-sm text-dc-ink">
                    <p><strong>Name:</strong> {selectedBooking.child?.name || 'Not available'}</p>
                    <p>
                      <strong>Date of Birth:</strong>{' '}
                      {selectedBooking.child?.dateOfBirth
                        ? new Date(selectedBooking.child.dateOfBirth).toLocaleDateString()
                        : 'Not available'}
                    </p>
                    <p><strong>Gender:</strong> {selectedBooking.child?.gender || 'Not available'}</p>
                  </div>
                </div>
        ...
        {showCancelModal && selectedBooking && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-dc-ink/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-[2rem] shadow-[0_20px_50px_-12px_rgba(74,144,164,0.35)] w-full max-w-md p-6">
              ...
              <h2 className="text-xl font-semibold font-baloo text-dc-ink mb-3">Cancel Visit Booking?</h2>
