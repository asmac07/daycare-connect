...
const ChatWindow = ({ daycareId, parentId, ownerId, otherUserName }) => {
  ...
  const otherUserId = String(user.id) === String(parentId) ? String(ownerId) : String(parentId)
  ...
  useEffect(() => {
    ...
    axiosInstance.get(`/messages/${daycareId}/${parentId}`)
      .then((res) => setMessages(res.data.data))
      .catch((err) => console.log(err))
    
      // to join a particular conversation room
    socket.emit('joinRoom', roomId)

    // checks the other user status
    if (otherUserId) {
      socket.emit('checkUserStatus', otherUserId)
    
    }

  // to setup socket listners
    ...
    socket.on('newMessage', (message) => {
      ...
      if (messageDaycareId === currentDaycareId && messageParentId === currentParentId) {
    ...
    // then unmount the component( to remove the old listner)
    return () => {
